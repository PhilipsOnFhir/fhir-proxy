...
public class CarePlanToCard {
    ...
    public static List<Card> convert(CarePlan carePlan, String fhirServerurl, String questionnaireServerUrl) throws FHIRException, FhirProxyNotImplementedException {
        ...
        for (CarePlan.CarePlanActivityComponent activity : carePlan.getActivity()) {
            ...
            if (activity.getReference() != null && activity.getReference().getReference().startsWith( "#" ) ) {
                ...
                throw new FhirProxyNotImplementedException( "Retrieval of repository stored request groups is not supported." );
            }
        }
        return cards;
    }

    private List<Card> convert( CarePlan carePlan, RequestGroup requestGroup, String fhirServerUrl, String questionnaireServerUrl ) throws FHIRException, FhirProxyNotImplementedException {
        List<Card> cards = new ArrayList<>();
                    ...
                    if ( subAction.hasType() && subAction.getType().getCode().equals("launch")){
                        ...
                        if ( subAction.hasResource()) {
                            ...
                            if ( subAction.getResource().getReference().startsWith(ResourceType.Questionnaire.name())){
                                String url = questionnaireServerUrl+subAction.getResource().getReference()+"?iss="+fhirServerUrl;
                                if ( carePlan.hasSubject() ){
                                    String prefix="Patient/";
                                    url+="&subject="+carePlan.getSubject().getReference();
                                }
                                link.setUrl( url );
                            } else {
...
//                }
    ...
    private Action createActionFromRequestGroupAction(CarePlan carePlan, RequestGroup.RequestGroupActionComponent requestGroupActionAction) throws FhirProxyNotImplementedException {
    ...
    private Resource getResource(CarePlan carePlan, Reference reference) throws FhirProxyNotImplementedException {
...
//                .filter( resource -> reference.getReference().substring( 1 ).equals( resource.getId() ) )
        ...
        throw new FhirProxyNotImplementedException( "Reference " + reference.getReference() + " does not point to a resource contained in Careplan" );
    }
