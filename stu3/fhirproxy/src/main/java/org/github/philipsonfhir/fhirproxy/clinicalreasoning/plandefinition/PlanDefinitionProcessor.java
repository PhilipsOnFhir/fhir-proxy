...
//    private static final Logger logger = Logger.getLogger(PlanDefinitionProcessor.class.getName());
    ...
    private static StructureMapTransformWorker structureMapTransformServer = null;
    ...
    public static void initialize(){
        ...
        if( structureMapTransformServer==null ){
            structureMapTransformServer = new StructureMapTransformWorker( );
        }
    }
    ///////////////////////////////////////

    public PlanDefinitionProcessor(BaseFhirDataProvider fhirDataProvider, PlanDefinition defRes, String patientId, String encounterId, String practitionerId, String organizationId, String userType, String userLanguage, String userTaskContext, String setting, String settingContext) {
        initialize();
...
//                    this.clinReasoningProvider.setValue( carePlan, dynamicValue.getPath(), result );
    ...
    private void processDefinition(PlanDefinition.PlanDefinitionActionComponent planDefinitionAction, RequestGroup.RequestGroupActionComponent requestGroupAction) throws FHIRException {
        IBaseResource defRes = null;
        if (planDefinitionAction.getDefinition().getReferenceElement().getIdPart().startsWith("#")) {
            defRes = resolveContained(planDefinition, planDefinitionAction.getDefinition().getReferenceElement().getIdPart() );
        }
        if ( planDefinitionAction.getDefinition().getResource() != null ) {
            defRes = planDefinitionAction.getDefinition().getResource();

        } else {
//            defRes = this.fhirDataProvider.getFhirClient().read().resource( ActivityDefinition.class ).withId( planDefinitionAction.getDefinition().getReferenceElement() ).execute();
            defRes = this.fhirDataProvider.getFhirClient()
                    .read()
                    .resource( planDefinitionAction.getDefinition().getReferenceElement().getResourceType() )
                    .withId( planDefinitionAction.getDefinition().getReferenceElement() )
                    .execute();
        }
        if ( defRes == null ) {
            throw new FHIRException( "Definition resource " + planDefinitionAction.getDefinition().getReferenceElement() + " cannot be found" );
        }

//        String definitionType = planDefinitionAction.getDefinition().getReferenceElement().getResourceType();
        Resource resource = (Resource)defRes;
        if ( resource instanceof ActivityDefinition ) {
            ...
            } catch (FHIRException | FhirProxyNotImplementedException e ) {
                throw new RuntimeException( "Error applying ActivityDefinition " + e.getMessage() );
            }
        } else if ( defRes instanceof StructureMap ){
            try {
                Resource result=null;
                StructureMap structuredMap = (StructureMap) defRes;
        ...
        } else if( defRes instanceof  PlanDefinition ) {
            try {
                PlanDefinitionProcessor planDefinitionProcessor =
                    new PlanDefinitionProcessor( fhirDataProvider, (PlanDefinition)defRes,
