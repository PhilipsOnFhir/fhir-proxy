...
import org.github.philipsonfhir.fhirproxy.clinicalreasoning.ClinicalReasoningImplementationGuide;
...
public class FhirProxyServletController extends HttpServlet {
    ...
    FhirProxyServletController(@Value("${proxy.fhirserver.url}") String fhirServerUrl, AsyncService asyncService ){
        ...
        fhirCallFactory.addImplementationGuide( new ClinicalReasoningImplementationGuide() );
    }
...
//            );
        ...
        try {
            ...
            if ( prefer!=null && prefer.equals("respond-async") ){
                ...
                String sessionUrl = callUrl.substring( 0, callUrl.indexOf( this.base+"/" ) ) + "/async-services/" + sessionId;
                resp.setStatus( 202 );
//                resp.addHeader( CONTENT_LOCATION, sessionUrl );
                resp.setHeader( CONTENT_LOCATION, sessionUrl );
                OperationOutcome operationOutcome = new OperationOutcome();
                operationOutcome.addIssue()
                        .setSeverity( OperationOutcome.IssueSeverity.INFORMATION )
